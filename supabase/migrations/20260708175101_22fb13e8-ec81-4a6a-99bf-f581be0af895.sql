...
BEGIN
  ...
  -- Bootstrap: allow the very first staff record on a library when the acting
  -- user is registering themselves as its owner (used by
  -- create_library_for_current_user during onboarding).
  IF TG_OP = 'INSERT'
     AND NEW.id = v_actor
     AND NEW.role = 'owner'
     AND NOT EXISTS (
       SELECT 1 FROM public.staff_users
       WHERE library_id = NEW.library_id AND id <> NEW.id
     ) THEN
    RETURN NEW;
  END IF;
